    ...
    <div className="min-h-screen bg-mesh">
      ...
      <div className="mx-auto max-w-3xl px-6 pb-24">
        ...
        <section className="mt-8">
          ...
          <h2 className="mt-2 font-serif text-2xl text-slate-900 sm:text-3xl">Two modes — and knowing which is which matters.</h2>
        ...
        <section className="mt-10">
          ...
          <h2 className="mt-2 font-serif text-2xl text-slate-900 sm:text-3xl">What to expect.</h2>
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            {[
              ...
              <div key={f.k} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-card">
                <div className="font-mono text-[9px] uppercase tracking-widest text-slate-400">{f.k}</div>
                <div className="mt-1 font-serif text-xl text-slate-900">{f.v}</div>
                <div className="mt-1 text-[12px] leading-snug text-slate-500">{f.s}</div>
              </div>
            ))}
          </div>
          <ul className="mt-4 space-y-px overflow-hidden rounded-2xl border border-slate-200 bg-white">
            {[
              ...
              <li key={t} className={`flex gap-3 px-4 py-3 ${i > 0 ? 'border-t border-slate-100' : ''}`}>
                <span className="mt-0.5 font-mono text-[11px] text-accent">{String(i + 1).padStart(2, '0')}</span>
                <span className="text-[13px] text-slate-600"><b className="text-slate-900">{t}</b> {d}</span>
              </li>
            ))}
          </ul>
        </section>
        ...
        <section className="mt-10">
          ...
          <h2 className="mt-2 font-serif text-2xl text-slate-900 sm:text-3xl">What this isn’t.</h2>
          <div className="mt-5 flex flex-wrap gap-2">
            {['Not graded', 'Not a personality type', 'Not a ranking', 'Not permanent', 'Not a verdict'].map((n) => (
              <span key={n} className="rounded-full border border-slate-200 bg-white px-4 py-1.5 text-[13px] text-slate-600">{n}</span>
            ))}
          </div>
        </section>

        {/* close */}
        <section className="mt-14 rounded-3xl bg-gradient-to-br from-white to-slate-50 p-10 text-center shadow-card ring-1 ring-slate-200">
          <p className="mx-auto max-w-md font-serif text-2xl italic leading-snug text-slate-800">
...
function ModeCard({ icon, tone, tag, title, children }: { icon: string; tone: 'amber' | 'accent'; tag: string; title: string; children: ReactNode }): JSX.Element {
  ...
  return (
    <div className="hover-lift rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
      <div className="mb-3 text-2xl">{icon}</div>
      <div className={`font-mono text-[9px] uppercase tracking-[0.1em] ${tagCls}`}>{tag}</div>
      <div className="mt-1 font-serif text-lg text-slate-900">{title}</div>
      <p className="mt-2 text-[13px] leading-relaxed text-slate-600">{children}</p>
    </div>
  );
}
