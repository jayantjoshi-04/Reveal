...
/** Public marketing landing page — nav · hero · about/vision · workflow · founders · footer. */
  ...
  return (
    <header className="sticky top-0 z-40">
      <div className="glass mx-auto mt-3 flex max-w-6xl items-center justify-between rounded-2xl px-5 py-3 sm:px-6">
        <span className="text-sm font-bold uppercase tracking-[0.3em] text-slate-900">
    ...
    <section className="relative overflow-hidden px-6 pb-24 pt-16 sm:pt-24">
      ...
      <div className="relative mx-auto max-w-4xl text-center">
        ...
        <div className="animate-slide-up mt-9 flex flex-wrap items-center justify-center gap-3" style={{ animationDelay: '240ms' }}>
          <button onClick={onStart} className="press rounded-2xl bg-accent px-7 py-3.5 text-[15px] font-semibold text-white shadow-lift transition-colors hover:bg-accent-dark">
            Get started →
          </button>
        ...
        <div className="animate-slide-up mt-16" style={{ animationDelay: '320ms' }}>
          <div className="glass mx-auto max-w-3xl rounded-3xl p-3 shadow-lift">
...
function ProgressPreview({ value }: { value: number }): JSX.Element {
  return (
    <div className="h-2 w-full overflow-hidden rounded-full bg-slate-200">
      <div className="h-full rounded-full bg-gradient-to-r from-accent to-violet-500" style={{ width: `${Math.round(value * 100)}%` }} />
    </div>
  );
}
...
function Workflow(): JSX.Element {
  const steps = [
    { n: '01', t: 'Read the intro, once', d: 'A short primer on how to answer — when to slow down, when to trust your gut.', snap: <SnapIntro /> },
    { n: '02', t: 'Section A · stated', d: 'Forced-choice reflections on capacities, values, conditions and aspiration.', snap: <SnapChoice /> },
    { n: '03', t: 'Section B · revealed', d: 'Quick behavioural tasks — budgets, dilemmas, a 40-artifact wish-sort.', snap: <SnapSort /> },
    { n: '04', t: 'Your Design Signature', d: 'A visual report you can revisit, search in your history, and re-run.', snap: <SnapReport /> },
  ];
  return (
    <section id="how" className="bg-slate-900 px-6 py-20 text-white">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-12 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {steps.map((s, i) => (
            <div key={s.n} className="animate-slide-up rounded-3xl border border-slate-700 bg-slate-800/50 p-4 backdrop-blur" style={{ animationDelay: `${i * 90}ms` }}>
              <div className="mb-4 aspect-[4/3] overflow-hidden rounded-2xl bg-slate-950/40 p-3">{s.snap}</div>
...
// structural survey snapshots (stylised, no real screenshots needed)
function SnapFrame({ children }: { children: ReactNode }): JSX.Element {
  return <div className="flex h-full flex-col rounded-xl bg-white p-3 text-slate-900">{children}</div>;
}
  ...
  return (
    <SnapFrame>
      ...
      <div className="mt-auto grid grid-cols-2 gap-1.5 pt-2">
        <div className="rounded-md bg-amber-50 px-2 py-1 text-[8px] font-medium text-amber-700">🕰 Slow down</div>
        <div className="rounded-md bg-accent-soft px-2 py-1 text-[8px] font-medium text-accent-dark">⚡ Go with gut</div>
      </div>
    </SnapFrame>
  );
}
    ...
    <SnapFrame>
      ...
      <div className="mt-2 space-y-1.5">
        {['talk to the people', 'work out the logic', 'picture how it looks'].map((t, i) => (
          <div key={t} className={`rounded-md border px-2 py-1 text-[8.5px] ${i === 0 ? 'border-accent bg-accent-soft text-accent-dark' : 'border-slate-200 text-slate-500'}`}>{t}</div>
        ))}
      </div>
    </SnapFrame>
  );
    ...
    <SnapFrame>
      ...
      <div className="mt-2 grid grid-cols-3 gap-1">
        {Array.from({ length: 9 }).map((_, i) => (
          <div key={i} className={`aspect-square rounded ${[1, 2, 5, 6].includes(i) ? 'bg-accent' : 'bg-slate-200'}`} />
        ))}
      </div>
    </SnapFrame>
  );
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-3">
          {people.map((p, i) => (
            <div key={p.name} className="hover-lift animate-slide-up rounded-3xl border border-slate-200 bg-white p-6 text-center shadow-card" style={{ animationDelay: `${i * 90}ms` }}>
              ...
              <div className="mx-auto flex h-28 w-28 items-center justify-center rounded-full bg-gradient-to-br from-accent-soft to-violet-100 ring-1 ring-slate-200">
...
// ── shared ───────────────────────────────────────────────────────────────────
function SectionLabel({ children, dark }: { children: ReactNode; dark?: boolean }): JSX.Element {
  return (
    <div className={`inline-flex items-center gap-2 font-mono text-[11px] uppercase tracking-[0.22em] ${dark ? 'text-indigo-300' : 'text-accent'}`}>
      <span className={`h-px w-6 ${dark ? 'bg-indigo-400/60' : 'bg-accent/50'}`} />
      {children}
    </div>
  );
}
