...
/** The full-bleed hero — a live "confusion → clarity" scene.
 *
...
export function ClarityHero({ onStart, onSignIn }: { onStart: () => void; onSignIn: () => void }): JSX.Element {
  ...
  return (
    ...
    >
      {/* ── ambient wash + grain ───────────────────────────────────────── */}
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(80%_110%_at_12%_50%,rgba(124,58,237,0.18),transparent_60%)]" />
      <div className="bg-grain pointer-events-none absolute inset-0 opacity-[0.11] mix-blend-soft-light" />
        ...
        >
          {THREADS.map((t, i) => (
            <path
              ...
              style={{ transformBox: 'fill-box', transformOrigin: 'center', animation: `thread-float ${t.dur}s ease-in-out ${t.delay}s infinite`, opacity: 0.9 }}
            />
          ))}
        </g>
      </svg>
      ...
      <div
        ...
        style={{
          ...
          background: 'linear-gradient(90deg,#fff 0%,rgba(255,255,255,0.7) 22%,rgba(168,120,248,0.35) 55%,transparent 100%)',
        }}
      >
      ...
      <div className="pointer-events-none absolute top-1/2 left-1/2 h-20 w-20 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white/60 blur-[40px]" style={{ transform: 'translate(calc(-50% + var(--mx) * 30px), calc(-50% + var(--my) * 20px))' }} />
      <div className="pointer-events-none absolute top-1/2 right-[7%] h-2 w-2 -translate-y-1/2 rounded-full bg-white shadow-[0_0_30px_10px_rgba(255,255,255,0.5)]" />
      ...
      <div className="relative z-10 grid min-h-[100svh] grid-rows-[1fr_auto_1fr] px-6">
        ...
        <div className="grid place-items-center">
          <div className="relative">
            <div className="relative flex h-[56px] min-w-[300px] items-center gap-2.5 rounded-full border border-white/25 bg-white/10 px-6 text-left shadow-[0_10px_44px_-8px_rgba(124,58,237,0.55)] backdrop-blur-xl sm:min-w-[360px]">
              <svg viewBox="0 0 24 24" className="h-4 w-4 flex-shrink-0 text-white/80" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
        ...
        <div className="flex flex-col items-center justify-start pt-9 text-center">
          <p className="animate-reveal-up mx-auto max-w-xl text-[15px] leading-relaxed text-white/60" style={{ animationDelay: '150ms' }}>
          ...
          <div className="animate-reveal-up mt-7 flex flex-wrap items-center justify-center gap-3" style={{ animationDelay: '230ms' }}>
            ...
            <button onClick={onSignIn} className="press rounded-2xl border border-white/15 bg-white/5 px-7 py-3.5 text-[15px] font-semibold text-white backdrop-blur transition-colors hover:border-white/30 hover:bg-white/10">
              Sign in
            </button>
          </div>
        </div>
      ...
      <button
        ...
        className="press absolute bottom-6 left-1/2 z-10 -translate-x-1/2 text-white/40 transition-colors hover:text-white/80"
      >
