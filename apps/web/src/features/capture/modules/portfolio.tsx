...
export function PortfolioFactsModule({ onSubmit, busy }: ModuleProps): JSX.Element {
  ...
  return (
    <>
      ...
      <div className="mb-4 space-y-5">
        {projects.map((p, i) => (
          <div key={i} className="rounded-2xl border border-slate-200 dark:border-white/10 bg-slate-50/50 dark:bg-white/[0.04] p-4">
            ...
            <div className="mb-3">
              <div className="mb-1.5 text-[13px] font-medium text-slate-700 dark:text-slate-200">Your role(s)</div>
            ...
            <div className="mb-4">
              <div className="mb-1.5 text-[13px] font-medium text-slate-700 dark:text-slate-200">Skills it shows</div>
              <div className="flex flex-wrap gap-1.5">
            ...
            <div className="mb-4">
              <div className="mb-1.5 text-[13px] font-medium text-slate-700 dark:text-slate-200">Did it lean commercial or impact?</div>
              <Segmented<Lean>
                value={p.lean}
            ...
            <div className="mb-3">
              <div className="mb-1.5 text-[13px] font-medium text-slate-700 dark:text-slate-200">How did it go?</div>
            ...
            <div>
              <div className="mb-1.5 text-[13px] font-medium text-slate-700 dark:text-slate-200">What was the setting like? <span className="font-normal text-slate-400">pick any that applied</span></div>
              <div className="flex flex-wrap gap-1.5">
      ...
      <button
        type="button"
        className="press mb-4 self-start rounded-xl border border-slate-200 dark:border-white/10 px-3.5 py-2 text-[13px] font-medium text-slate-600 dark:text-slate-300 transition-colors hover:border-slate-300"
...
export function PortfolioInterpretiveModule({ onSubmit, busy }: ModuleProps): JSX.Element {
  ...
  return (
    <>
      ...
      <p className="mb-3 text-sm text-slate-500 dark:text-slate-400">Asked now, after the tasks, so it can’t anchor everything else.</p>
      <textarea
        className="mb-6 h-24 w-full resize-none rounded-2xl border border-slate-200 dark:border-white/10 bg-white dark:bg-noir-card px-4 py-3 text-sm text-slate-900 dark:text-white shadow-sm transition-colors placeholder:text-slate-400 focus:border-accent focus:outline-none focus:ring-2 focus:ring-accent/15"
        value={text}
        onChange={(e) => setText(e.target.value)}
        placeholder="Mostly researching and making sense of the mess…"
      />

      <div className="mb-2 text-[13px] font-medium text-slate-700 dark:text-slate-200">Now think of a project you <b>struggled</b> with, or that didn’t go well — it doesn’t have to be in your portfolio.</div>
      <div className="mb-4">
        <Segmented<'yes' | 'no'>
      ...
      {present ? (
        <div className="mb-5 animate-fade-in">
          <div className="mb-1.5 text-[13px] font-medium text-slate-700 dark:text-slate-200">What was the setting like? <span className="font-normal text-slate-400">not why it went wrong — just the room</span></div>
...
export function ResumeModule({ onSubmit, busy }: ModuleProps): JSX.Element {
  ...
  return (
    <>
      ...
      <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">How you package yourself, compared against everything you just showed us.</p>
      <div className="mb-5">
        <UploadField
      ...
      <div className="mb-2 text-[13px] font-medium text-slate-700 dark:text-slate-200">How does it read?</div>
