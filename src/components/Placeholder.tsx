...
type PlaceholderProps = {
  ...
  height?: number;
  /** Illustration shown floating above the title. */
  image?: string;
};
 ...
 */
export function Placeholder({ eyebrow, title, blurb, height = 1391, image = A.home.seedCluster }: PlaceholderProps) {
  return (
    <FixedCanvas height={height}>
      ...
      <div className="absolute left-1/2 top-[280px] h-[150px] w-[120px] -translate-x-1/2">
        <div className="h-full w-full animate-float">
          <Asset src={image} alt="" placeholder="blank" />
        </div>
      </div>
